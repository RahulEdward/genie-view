...
const DEFAULT_WS_HOST = '127.0.0.1:8000/ws';
...
class SharedWebSocketManager {
    ...
    _ensureConnected() {
        ...
        let url;
        if (isLocalDev) {
            // Use Vite proxy - connect to same host as frontend
...
/**
 * Get Login URL
 * For API-only backend, we handle auth in frontend
 * Returns current page URL to trigger re-render with login form
 */
